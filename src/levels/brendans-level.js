...
export function createLevel(world) {
    ...
    //Create platforms
    createPlatform(world, 8,10,20,ruined_forest); //Main large bottom platform
    createPlatform(world, -25, 7, 10, ruined_forest); //Left platform where box lands
    createPlatform(world, -10, 8, 1, ruined_forest); //single left platform to jump on before box platform
    createPlatform(world, 9, 2, 5, ruined_forest); //Upper single platform over main spawn
    createPlatform(world, -35, 5, 1, ruined_forest); //single platform to hold box from falling off
    createPlatform(world, -12, -5, 10, ruined_forest); //platform with door on it
    createBox(world, Vec2(-28,2),2,2, 10);
    createPlatform(world, -20, -2, 1, ruined_forest); //single platform attached to door plat left
